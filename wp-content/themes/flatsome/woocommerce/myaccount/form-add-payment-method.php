	...
	<div id="payment">
		<?php if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) : ?>
			...
			<div class="form-row">
				...
				<input type="submit" class="button alt" id="place_order" value="<?php esc_attr_e( 'Tambahkan Metode Pembayaran', 'woocommerce' ); ?>" />
				<input type="hidden" name="woocommerce_add_payment_method" value="1" />
			</div>
		<?php else : ?>
			<p><?php _e( 'Maaf ,sepertinya tidak ada metode pembayaran yang mendukung penambahan metode pembayaran baru . Silakan hubungi kami jika Anda memerlukan bantuan atau ingin membuat pengaturan alternatif .', 'woocommerce' ); ?></p>
		<?php endif; ?>
	</div>
</form>
