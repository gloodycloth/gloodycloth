	...
	if ( ! class_exists( 'WC_apm' ) ) {
		...
		class WC_apm {
			...
			public function __construct() {
				...
				$this->settings = array(
					array(
						'name' => __( 'Metode Pembayaran yang Diterima', 'woocommerce-accepted-payment-methods' ),
						'type' => 'title',
						'desc' => sprintf( __( 'Untuk menampilkan metode pembayaran yang dipilih Anda dapat menggunakan built in widget, the %s shortcode atau the %s template tag.', 'woocommerce-accepted-payment-methods' ), '<code>[woocommerce_accepted_payment_methods]</code>', '<code>&lt;?php wc_accepted_payment_methods(); ?&gt;</code>' ),
						'id' => 'wc_apm_options'
					),
					array(
	...
	class Accepted_Payment_Methods extends WP_Widget {
		...
		public function form( $instance ) {
			if ( isset( $instance[ 'title' ] ) ) {
				...
				$title = __( 'Metode Pembayaran Diterima', 'woocommerce-accepted-payment-methods' );
			}
			?>
			<p>
